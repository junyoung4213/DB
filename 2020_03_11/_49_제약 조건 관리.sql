...
--  이럴 때 제약조건을 비활성화하고 데이터 입력 후
--  다시 제약조건을 활성화한다.
--  만일 제약조건에 위배되는 데이터가 있다면 쿼리로
--  검사해서 발견할 수 있다.
--2) 사이트 초기 개발시
--  부모-자식 테이블의 관계가 설정되어 있으면
--  테이블의 단위테스트에서 데이터 입력이 번거로울 수 있다.
--  개발자들은 초기 개발시 일시 비활성화하여 테스트를
--  진행하는 경우가 종종 있다.
--  물론 나중에 다시 활성화해야 한다.
--  
--  <제약 조건 추가/삭제>
...
--제약 조건 삭제
--CASCADE를 쓰면 CLASS_CNO_PK 조건이 삭제되고
--관련된 참조조건인 ST_CNO_FK 조건도 삭제된다.

--만약 CASCADE 를 안쓰면 CLASS_CNO_PK 삭제가 안된다.
--왜냐하면 자식테이블이 FK조건으로 참조하고 있기 때문이다.
ALTER TABLE CLASS
DROP CONSTRAINT CLASS_CNO_PK; --CASCADE // 참조되고 있다면 삭제가 안된다.

ALTER TABLE CLASS
DROP CONSTRAINT CLASS_CNO_PK CASCADE;   -- 참조되고 있어도 삭제됨. 
...
--부모 CLASS 테이블의 PK, 자식 ST테이블의 FK 설정 제약조건
ALTER TABLE CLASS
ADD CONSTRAINT CLASS_CNO_PK PRIMARY KEY(CNO);
ALTER TABLE ST
ADD CONSTRAINT ST_CNO_FK FOREIGN KEY(CNO)
...
--  - 온라인 처리중이 아니면 SHARED POOL의 크기를 임시로
--    줄인다.
