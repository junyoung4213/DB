...
-- 구조가 변경된다는 것은 사실상 요구 분석과모델링에 문제가
-- 있었다는 의미로 판단한다.
--  
...
AS (SELECT dno, eno, ename, sal*12+NVL(comm,0)
    FROM emp
    WHERE 1=2); --데이터를 넣지 않기 위해 성립되지 않는 조건을 써주었다.
    --빈 테이블을 생성할 때 그냥 CREATE TABLE을 사용하지 않는 이유는,
    --순수하게 CREATE TABLE을 사용했을 시 자료형을 일일히 다 써주어야 되기 때문이다.
