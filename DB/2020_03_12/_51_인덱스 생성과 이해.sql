...
-- 중고급으로 올라갈수록, SQL 에 대한 이해도가 높아질수록
-- 인덱스에 대한 활용도가 올라간다.
...
행을 DB전체에 걸쳐서 유일하게 구별하며 I/O를 수행할 때
위치를 직접 알려줌으로 가장 빠르게 행에 접근하는 정보이다. 
구성은 저장된 파일의 번호, 블록 번호, 행 번호, 객체 번호 등으로
구성되며 모두 RDBMS가 직접 사용하도록 64bit로 
엔코딩되어 있다.

<인덱스의 장점과 단점>
인덱스는 테이블을 설계하는 시점부터 효과적인 인덱스 이용을 위해
생성이 계획되고 응용 프로그램 설계에 반영돼야 한다.
인덱스 테이블에 대한 DML이나 DDL 작업이 수행되면 RDBMS 가
자동으로 유지해준다.
...
5) FK 컬럼에 반드시 인덱스를 생성해준다.
  ...
  - FK는 조인에 사용되므로 PK나 UK에만 인덱스를 만들어놓고
   FK에는 안 만들면, 조인시 FK가 있는 테이블은 '풀 테이블 스캔'을 
   하기 때문에 조인 검색속도가 인덱스의 효과를 보기 어렵다.
...
ON 테이블명 (컬럼명|함수|수식);
  ...
  2) 함수나 수식을 사용하는 인덱스(함수 기반:Function base)
    도 생성 가능하다
  3) 제약 조건에 의해 자동으로 만들어진 인덱스는 제약조건과
    이름이 동일하다.
