...
--  시퀀스(Sequenece)등 여러 개체가 있고 이들 중에
...
--          date_type, data_length
--   FROM user_tab_columns --sys 계정의 소유 테이블
--   WHERE talbe_name = 테이블명;
...
--co1 CHAR(4)     'AA'    AA__ 
----CHAR는 4byte의 자리를 줬을때 2자리만 사용하면 2자리를 남긴다
--co2 VARCHAR2(4) 'AA'    AA   
----VARCHAR2는 크기가 가변적이기 때문에 2자리 사용하면 2자리만 사용함
...
--아래 방법은 비추천
--왜냐하면 hdate를 문자열로 자동 형변환하므로
--대용량 테이블에서는 심각한 성능 저하가 일어날 수 있다.
SELECT *
FROM hd
WHERE hdate LIKE '20/02/27%';

1) 우리나라에서는 날짜를 DATE 보다 VARCHAR2(8)
   이렇게 정의하는 경우가 많다.
   대신 시분초를 사용할 수 없게 된다.
2) 외국에서는 
   TRUNC(sysdate) 를 사용해서 시분초를 항상 00:00:00이 되게 저장한다.
